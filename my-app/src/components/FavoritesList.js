...
const FavoritesList = (props) => {
    ...
    const [posterBeingAdded, setPosterBeingAdded] = useState(null);
    ...
    // if (showFaves) {
    return (
    <div className={`mt-8 mr-10 right-0 top-0 transition-all duration-500 ease-in-out ${showFaves ? 'opacity-100' : 'opacity-0 translate-x-full'}`}>
      ...
      <ul>
        {favMovies.map((movie) => {
            return (
                <li key={movie.id} className="mt-4" onMouseEnter={()=>setShowHiX(true)} onMouseLeave={()=>setShowHiX(false)}>
